...
def rTE_forward(frequencies, lamb, sigma, chi, thicknesses):
    ...
    """
    # Sigma and Chi must be fortran contiguous
    sigma = np.require(sigma, dtype=np.complex128, requirements="F")
    chi = np.require(chi, dtype=np.float64, requirements="F")
    ...
    if chi.shape != sigma.shape:
        raise ValueError(
            f"chi array must match sigma array shape. Got {chi.shape} and {sigma.shape}"
        )
    ...
    rTE(out_p, &f[0], &lam[0], sig_p, &ch[0, 0], h_p,
          n_frequency, n_filter, n_layers)

    return np.array(out)

def rTE_gradient(frequencies, lamb, sigma, chi, thicknesses):
    """Compute reflection coefficients for Transverse Electric (TE) mode.
    ...
    cdef COMPLEX_t[:, :, :] gchi = np.empty((n_layers, n_frequency, n_filter), dtype=np.complex128, order="F")

    cdef:
        # Types are same size and both pairs of numbers (r, i), so can cast one to the other
        complex_t *sig_p = <complex_t *> &sig[0, 0]
        complex_t *gsig_p = <complex_t *> &gsig[0, 0, 0]
        complex_t *gchi_p = <complex_t *> &gchi[0, 0, 0]
        complex_t *gh_p = NULL
        REAL_t *h_p = NULL
    if n_layers > 1:
        h_p = &hs[0]
    ...
    rTEgrad(gsig_p, gchi_p, gh_p, &f[0], &lam[0], sig_p, &ch[0, 0], h_p,
          n_frequency, n_filter, n_layers)

    return np.array(gsig), np.array(gh), np.array(gchi)
