...
class EarthquakeInterferogram(properties.HasProperties):
    title = properties.String("name of the earthquake", required=True)

    description = properties.String("description of the event", required=False)

    location = properties.Vector2(
        "interferogram location (bottom N, left E)", required=True
    )

    location_UTM_zone = properties.Integer("UTM zone", required=True)

    shape = properties.Array(
        "number of pixels in the interferogram", shape=(2,), dtype=int, required=True
    )

    pixel_size = properties.Array(
        "Size of each pixel (northing, easting)", shape=(2,), dtype=float, required=True
    )

    data = properties.Array(
        "Processed interferogram data (unwrapped)", dtype=float, required=True
    )

    ref = properties.Vector2("interferogram reference", required=True)

    ref_incidence = properties.Float("Incidence angle", required=True)

    scaling = properties.Float("Scaling of the interferogram", default=1.0)

    satellite_name = properties.String("Name of the satelite.")

    satellite_fringe_interval = properties.Float("Fringe interval", default=0.028333)

    satellite_azimuth = properties.Float("satellite_azimuth", required=True)

    satellite_altitude = properties.Float("satellite_altitude", required=True)

    local_rigidity = properties.Float("Local rigidity", default=3e10)

    local_earth_radius = properties.Float("Earth radius", default=6371000.0)

    date1 = properties.DateTime("date1", required=True)

    date2 = properties.DateTime("date2", required=True)

    processed_by = properties.String("processed_by", required=True)

    processed_date = properties.DateTime("processed_date", required=True)

    copyright = properties.String("copyright", required=True)

    data_source = properties.String("data_source", required=True)
    ...
    def _get_plot_data(self):
        ...
        vectorNx = (
            np.r_[0, np.cumsum((self.pixel_size[0],) * self.shape[0])]
            + self.location[0]
        )
        vectorNy = (
            np.r_[0, np.cumsum((self.pixel_size[1],) * self.shape[1])]
            + self.location[1]
    ...
    def plot_interferogram(self, wrap=True, ax=None):
        ...
        ax.axis("equal")
        ax.set_xlabel("Easting, m (UTM Zone {})".format(self.location_UTM_zone))
    ...
    def plot_mask(self, ax=None, opacity=0.2):
        ...
        out = ax.pcolormesh(
            vectorNx, vectorNy, np.ma.masked_where(~np.isnan(data), data), cmap=cmap
        )
    ...
    def _ang_to_gc(x, y, origx, origy, satAzimuth):
        ...
        Ngc[0] = (np.sin(satAzimuth) * np.sin(origy) * np.cos(origx)) - (
            np.cos(satAzimuth) * np.sin(origx)
        )
        Ngc[1] = (np.sin(satAzimuth) * np.sin(origy) * np.sin(origx)) + (
...
class Oksar(properties.HasProperties):
    ...
    length = properties.Float("Fault length", default=10000.0, min=0)
    ...
    O = properties.Vector2("Origin of the simulation domain", required=True)

    U = properties.Vector2("U direction of the simulation domain", required=True)

    V = properties.Vector2("V direction of the simulation domain", required=True)
    ...
    def _dc3d3(self, alpha, X, Y, dip, al1, al2, aw1, aw2, disl1, disl2):
        ...
        for k in [1.0, 2.0]:
            ...
            for j in [1.0, 2.0]:
                ...
                if np.any(ret < 1e-14):
                    raise Exception(
                        "dccon2 b %f %f %f %f" % (ret, c2_r, dc_et, dc_q, dc_xi)
                    )
                ...
                if np.any(rd < 1e-14):
                    raise Exception(
                        "ub %f %f %f %f %f %f" % (rd, c2_r, c2_d, xi, et, q)
                    )
                ...
                if c0_cd != F0:
                    ...
                    ai4 = (
                        ...
                        * (
                            ...
                            + F2
                            * np.arctan(
                                (et * (xx + q * c0_cd) + xx * (c2_r + xx) * c0_sd)
                                / (xi * (c2_r + xx) * c0_cd)
                            )
                        )
                    )
    ...
    def plot_displacement(self, eq=None, ax=None, wrap=True, mask_opacity=0.2):
        ...
        vectorNx = (
            np.r_[0, np.cumsum((self.U[0] / self.shape[0],) * self.shape[0])]
            + self.O[0]
        )
        vectorNy = (
            np.r_[0, np.cumsum((self.V[1] / self.shape[1],) * self.shape[1])]
            + self.O[1]
        )
...
def example():
    ...
    dinar_file = requests.get("http://storage.googleapis.com/sci3pt-insar/dinar.r4")
